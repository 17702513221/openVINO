...
// 将BGR24原始格式输出到控制台.
void outputFrame(const cv::Mat &frame) {
    int i,j;
    unsigned char b, g, r;
    cv::Vec3b pixel;
    for(int j = 0;j < frame.rows;j++){
      for(int i = 0;i < frame.cols;i++){
          pixel = frame.at<cv::Vec3b>(j, i);
            ...
            if (OK == async_infer_request_curr->Wait(IInferRequest::WaitMode::RESULT_READY)) {
                ...
                for (auto &object : objects) {
                    ...
                    if (confidence > FLAGS_t) {
                        ...
                        //输出结果
			//slog::info << std::to_string(label) << slog::endl;
                        //使用人脸识别平台测试
                        publishMQTTMessage("yolov3/results", std::to_string(label));
                        cv::putText(frame,
