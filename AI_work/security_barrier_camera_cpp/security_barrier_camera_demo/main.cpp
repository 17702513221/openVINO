...
struct LPRDetection : BaseDetection {
    ...
    std::string GetLicencePlateText(BaseInferRequest::Ptr request) {
        ...
        publishMQTTMessage("carid/results", result);
        return result;
    }
    CNNNetwork read() override {
...
int main(int argc, char *argv[]) {
	...
	int result = mqtt_start(handleControlMessages);
        if (result == 0) {
            slog::info << "MQTT started." << slog::endl;
        } else {
            slog::info << "MQTT NOT started: have you set the ENV varables?" << slog::endl;
        }
        ...
        do {
                ...
                for (; pidx < results.size(); pidx++) {
                    if (results[pidx].label == 1) {  // vehicle
                        if (VehicleAttribs.enabled()) {
                            ...
                            wchar_t *w_str;
                            ToWchar(str,w_str);
                            text.putText(frame, w_str, cv::Point(550,20), cv::Scalar(0, 0, 0));
                            if (FLAGS_auto_resize) {
                                ROI cropRoi;
            ...
            cv::putText(displayImage.getMat(), out.str(), cv::Point2f(630, 20), cv::FONT_HERSHEY_TRIPLEX, 0.7,
                        cv::Scalar(0, 0, 0));
